...
G4VPhysicalVolume* GeMSE_DetectorConstruction::Construct()
{
  ...
  G4Material* Teflon = new G4Material("Teflon", density=2.20*g/cm3, ncomponents=2);
  ...
  */
  ...
  // +++ MOVABLE TOP PART +++
      ...
      G4Box* innerPbShieldingBackMov_box = new G4Box("innerPbShieldingBackMov_box",xSizeInnerPbShieldingBack_mov/2., ySizeInnerPbShieldingBack_mov/2., zSizeInnerPbShieldingBack_mov/2.);
  ...
  */
  ...
  // place volumes
  //new G4PVPlacement    (0,G4ThreeVector(0.,0.,zPosVacuumDet),VacuumDet_log,"VacuumDet",CuHsg_log,false,0);
  ...
  // G4SubtractionSolid* CuFilling = new G4SubtractionSolid("CuFilling", SampleCavity_box, CuHsg_tube_filled, 0, G4ThreeVector(0.,0.,-zSizeSampleChamber/2.));
  // // define logical volumes
  // G4LogicalVolume* CuFilling_log = new G4LogicalVolume(CuFilling, copper_mat, "CuFilling_log", 0, 0, 0);

  // // place volumes
  // new G4PVPlacement(0,G4ThreeVector(0.,0.,zPosCuShielding+zSizeCuShielding/2.-zSizeSampleChamber/2.+0.001*cm),CuFilling_log,"CuFilling",expHall_log,false,0);
  ...
  G4Material* chondrite_EH = new G4Material("sample_mat", density=3.1*g/cm3, ncomponents=10); //chondrite_EH
  ...
  //CADMesh *mesh_top = new CADMesh("../sample_geometries/apdRingOben_ohneLoch_fein_ascii.STL", mm, sample_pos, false);
  //CADMesh *mesh_top = new CADMesh("sample_geometries/test_basalt5.stl", mm, sample_pos, false);
  //banana
  // G4ThreeVector sample_pos = G4ThreeVector(-25,20,-3.*mm);
  // CADMesh *mesh_top = new CADMesh("sample_geometries/banana.stl", mm, sample_pos, false);
  //
  // sample_solid = mesh_top->TessellatedMesh();
  // sample_logical = new G4LogicalVolume(sample_solid, chondrite_EH, "sample_logical", 0, 0, 0);
  // sample_physical = new G4PVPlacement(G4Transform3D(rmr, sample_pos) , sample_logical, "sample",expHall_log, false, 0);
...
void GeMSE_DetectorConstruction::OverlapCheck()
{
  ...
  G4bool overlapFlag = false;   // Boolean variable overlapFlag, initialized to 'false'

  for (size_t i=0; i<thePVStore->size();i++)    overlapFlag = (*thePVStore)[i]->CheckOverlaps(5000) | overlapFlag;
