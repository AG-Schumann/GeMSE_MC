...
class CADMesh {
 public:
  ...
  CADMesh(char* file_, char* type_, double scale_, G4ThreeVector offset_,
          G4bool reverse_);
 ...
 public:
  ...
  // TODO: This method will be removed in a future version. Use
  // CADMesh::GetFileName instead.
  G4String MeshName() { return file_name; };

  G4String GetFileName() { return this->file_name; };

  G4String GetFileType() { return this->file_type; };

  void SetVerbose(G4int verbose_) { this->verbose = verbose_; };

  G4int GetVerbose() { return this->verbose; };

  void SetScale(G4double scale_) { this->scale = scale_; };

  G4double GetScale() { return this->scale; };

  void SetOffset(G4ThreeVector offset_) { this->offset = offset_; };

  G4ThreeVector GetOffset() { return this->offset; };

  void SetReverse(G4bool reverse_) { this->reverse = reverse_; };

  G4bool GetReverse() { return this->reverse; };

  void SetMaterial(G4Material* material_) { this->material = material_; };

  G4Material* GetMaterial() { return this->material; };

  void SetQuality(G4double quality_) { this->quality = quality_; };

  G4double GetQuality() { return this->quality; };

  G4AssemblyVolume* GetAssembly() { return assembly; };

  tetgenio GetTetgenInput() { return in; };

  tetgenio GetTetgenOutput() { return out; };
